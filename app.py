...
def edit_reminder(reminder_id):
    ...
    new_time=st.time_input("Update_time",reminder["date_time"].time())
    ...
    if st.button("Update Reminder"):
        ...
        reminders_collection.update_one({"_id":reminder_id},
        {"$set":updated_data})
...
if reminders:
    ...
    st.dataframe(df.style.set_properties(**{'background-color': '#FFE4E1', 'color': 'black'}))  # Light Pink Theme
else:
...
# Perform search only if a term is entered
if search_term:
...
for reminder in reminders:
    with st.expander(f"Reminder: {reminder['type']}"):
        ...
        st.button("Edit", key=reminder["_id"], on_click=edit_reminder, args=(reminder["_id"],))
